...
var
	...
	expect = chai.expect,
	should = chai.should(),
	BloomFilter = require('../lib/mmapped')
	;
...
describe('MMappedFilter()', function()
{
	it('constructs a filter of the requested size', function()
	{
		var filter = new BloomFilter( "/tmp/bloom1", { hashes: 4, bits: 32, create: true } );
	...
	{
		var filter = new BloomFilter( "/tmp/bloom1", { hashes: 3, bits: 64, create: true } );
	...
	{
		var filter = new BloomFilter( "/tmp/bloom1", { bits: 256, seeds: [1, 2, 3, 4, 5], create: true });
	...
	describe('createOptimal()', function()
	{
		...
		{
			var filter = BloomFilter.createOptimal( "/tmp/bloom1", 95);
			filter.bits.should.equal(1048);
			filter.hashes.should.equal(8);

			filter = BloomFilter.createOptimal( "/tmp/bloom1", 148);
			filter.bits.should.equal(1632);
			filter.hashes.should.equal(8);

			filter = BloomFilter.createOptimal( "/tmp/bloom1", 10);
		...
		{
			var filter = BloomFilter.createOptimal( "/tmp/bloom1", 20000);
			filter.bits.should.equal( 220555);
			var previous = filter.bits;

			filter = BloomFilter.createOptimal( "/tmp/bloom1", 20000, 0.2);
	...
	describe('setbit() and getbit()', function()
	{
		...
		{
			var filter = new BloomFilter("/tmp/bloom1", { hashes: 3, bits: 16, create: true });
		...
		{
			var val;
			var filter = new BloomFilter( "/tmp/bloom1", { hashes: 3, bits: 64, create: 1 });
	...
	describe('add()', function()
	{
		...
		{
			var filter = new BloomFilter( "/tmp/bloom1", { hashes: 4, bits: 128, create: 1 });
		...
		it('can add a hundred random items', function()
		{
			...
			var filter = BloomFilter.createOptimal( "/tmp/bloom1", 100 );
			var words = [];
			for (var i = 0; i < 100; i++)
			{
				var w = randomWord();
	...
	describe('has()', function()
	{
		...
		{
			var filter = new BloomFilter( "/tmp/bloom1", { hashes: 4, bits: 50, create: true } );
			filter.add('cat');
		...
		{
			var filter = new BloomFilter( "/tmp/bloom1", { hashes: 3, bits: 128, create: true } );
			filter.add(['cat', 'dog', 'wallaby']);
