...
describe('CountingFilter()', function()
{
	...
	{
		var filter = new CountingFilter(4, 32);
	...
	describe('optimalForSize()', function()
	{
		...
		{
			var filter = CountingFilter.optimalForSize(95);
	...
	describe('setbit()', function()
	{
		...
		{
			var filter = new CountingFilter(3, 16);
			filter.setbit(0);
	...
	describe('has()', function()
	{
		...
		{
			var filter = new CountingFilter(3, 64);
			filter.add(['cat', 'dog', 'wallaby']);
	...
	describe('remove()', function()
	{
		...
		{
			var filter = new CountingFilter(4, 128);
			filter.add(['cat', 'dog', 'wallaby']);
			filter.has('cat').should.equal(true);
			filter.remove('cat');
		...
		it('can add and then remove a hundred random items', function()
		{
			...
			var filter = CountingFilter.optimalForSize(100);
			var words = [];
			for (var i = 0; i < 100; i++)
			{
				var w = randomWord();
