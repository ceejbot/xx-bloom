...
BloomFilter.prototype.toBuffer = function toBuffer()
{
	...
	// first 2 bytes: uint 16 containing # of bits
	// third byte: number of hash seeds, N (note lurking bug if ludicrous # of seeds)
	...
	const buf = Buffer.alloc(2 + 1 + this.seeds.length * 4 + this.buffer.length);

	var ptr = 0;
	buf.writeUInt16LE(this.bits, ptr);
	ptr += 2;
...
{
	var ptr = 0;
	this.bits = buf.readUInt16LE(ptr);
	ptr += 2;
	...
	this.buffer = Buffer.alloc(buf.length - 3 - (4 * seedcount));
